# Player Profile Template

## Identity and continuity

- Player name:
- Preferred assistant name, if any:
- Time available for vault maintenance:
- Self-described inventory habits (collector, minimalist, pack rat, uncertain, other):
- Explicit goals for the project:
- Statements or preferences that must be preserved verbatim or nearly verbatim:

## Class and input

- Currently active class:
- Other classes actually played:
- Preferred subclasses:
- Favorite Exotic armor:
- Controller or mouse and keyboard:

## Class/build weapon interactions

- Class ability normally equipped:
- Instant reload sources:
- Passive/holstered reload sources:
- Magazine-overflow or sustain loops:
- Reload-triggered weapon perks relied upon:
- Ability, Exotic, or artifact interactions that change weapon value:
- Build-specific weapon types/elements required:

## Activity priorities

Rank these:

- Grandmasters / Conquests:
- Raids:
- Dungeons:
- Solo / solo flawless:
- General PvE:
- PvP / Trials:
- Farming / speed:

## Gear access

- Dungeon keys or dungeons owned:
- Raids regularly accessible:
- Trials participation:
- Master/Adept farming willingness:
- Willingness to reacquire difficult gear:
- Activities the player cannot or will not farm:

## Playstyle

- Preferred engagement range:
- Favorite weapon types:
- Favorite intrinsic frames/archetypes:
- Frames/archetypes to avoid:
- Shotgun preference: slug, pellet, Rapid-Fire, Precision, other:
- Other important frame preferences:
- Origin traits or foundries valued:
- Avoided weapon types:
- Simple reliable loops or high-execution rotations:
- Survivability, support, control, burst, sustained damage, or speed:
- Aggressive or conservative positioning:

## Vault cleanup

- Current vault count:
- Desired free slots:
- Acceptable deletion risk:
- Protect all crafted items?
- Protect all raid/dungeon items?
- Protect high kill-count items?
- Sentimental items or custom DIM tags:

## Personal evidence ledger

For each meaningful statement or correction, record:

- Date or approximate session:
- What the player explicitly said:
- Weapon, build, class, or activity context:
- Durable preference or temporary build state:
- Analytical consequence:
- Superseded by a later statement?
