# Brent's Destiny 2 AI Bootstrap v2.4.1
...
## v2.4.1 patch summary

- Adds a permanent recoil-direction mechanics reference and supporting damped-sine graph.
- Defines the final-stat model `B(x) = sin((x + 5)π / 10) × (100 - x)` as an explanatory community model rather than verified source code.
- Records centered nodes ending in 5, the special ideal endpoint at 100, alternating side bias, and the shrinking bias envelope near 100.
- Requires final configured Recoil Direction, Stability, archetype, input method, and perk/stat tradeoffs to be considered together.
- Prevents simplistic deletion logic based only on “higher is better” or “ending in 5 is always better.”
- Adds explicit `+15` examples and vault-analysis safeguards.
...
The permanent detailed reference is:

`docs/Recoil-Direction-Mechanics.md`

The supporting graph is:

`docs/assets/Recoil-Direction-Damped-Sine-Graph.png`

Recoil Direction must not be reduced to either “higher is always better” or “the final digit is all that matters.”

### 9.1 What the stat controls

- **Recoil Direction** primarily controls horizontal recoil tendency and directional bias.
- **Stability** controls the severity of kick, bounce, sustained grouping, and recovery behavior.
- **Accuracy** controls how closely shots follow the intended point and accuracy cone.

A weapon can have excellent Recoil Direction and still jump sharply because its Stability is poor.

### 9.2 Damped-sine explanatory model

Use the traditional community model:
...
- `x` is the **final configured Recoil Direction stat**;
- the sign of `B(x)` represents opposite left/right tendencies;
- `|B(x)|` represents the relative strength of that directional bias;
- `(100 - x)` shrinks the possible bias envelope as the stat approaches 100.

Treat this as a durable explanatory model, not proof that the equation is literally Bungie's current game-code implementation. Reverify patch-specific exceptions.

### 9.3 Centered nodes and 100

The sine term crosses zero at:

```text
5, 15, 25, 35, 45, 55, 65, 75, 85, 95
```

These ending-in-5 values have no preferred left/right direction in the model. **100 is also centered and is the ideal endpoint** because `(100 - x)` reaches zero.

Values ending in 0 lie near alternating side-bias peaks, but the peaks shrink as the stat rises:

| Final stat | Model value | Interpretation |
|---:|---:|---|
| 20 | +80 | Very strong side tendency |
...
The sign tells which side; the absolute magnitude is normally more important for comparison.

### 9.4 Why bonuses are not monotonically more vertical

A higher stat generally reduces the maximum possible horizontal bias, but a bonus can move the final number away from a centered node.

Examples with `+15` Recoil Direction:

| Starting stat | Final stat | Result |
|---:|---:|---|
| 65 | 80 | Higher stat, but moves from centered to a modest side bias |
...
Always evaluate the **final number**, not merely the size of the increase.

### 9.5 The 55-versus-90 lesson

- 55 is mathematically centered in the model.
- 90 has a slight side preference but a much smaller remaining bias envelope because it is close to 100.

Do not declare 55 universally better because it ends in 5. Do not declare 90 universally better merely because it is higher. Evaluate the complete weapon, Stability, archetype, input method, and actual feel.

### 9.6 Vault-analysis rules

For close comparisons:

1. Calculate final Recoil Direction with each useful selectable barrel/mod configuration.
2. Enumerate configurations on Tier 3/4/5 weapons rather than grading only the currently selected barrel.
3. Evaluate Stability, Range, Handling, magazine, zoom, archetype, input method, and weapon-specific tuning alongside it.
4. Weight Recoil Direction most on autos, SMGs, pulses, machine guns, other sustained-fire weapons, and PvP consistency rolls.
5. Weight it less on slow reacquisition weapons and projectile frames such as rocket-assisted sidearms.
6. Use it as a tie-breaker; never erase a unique or defining perk role solely for a prettier number.
7. State explicitly when a recoil bonus moves a weapon from centered to side-biased or requires sacrificing valuable stats.
8. Do not overvalue moving an already excellent 85 or 95 to 100 when the cost is a defining perk or materially better stat package.
9. Test weapon-specific exceptions when practical.

**100 Recoil Direction does not mean zero recoil, zero spread, or high Stability.** Vertical climb, accuracy variation, projectile behavior, and large low-Stability jumps can remain.
